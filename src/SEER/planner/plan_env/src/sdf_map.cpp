...
void SDFMap::initMap(ros::NodeHandle& nh) {
  ...
  nh.param("sdf_map/map_origin_x", x_origin, -1.0);
  nh.param("sdf_map/map_origin_y", y_origin, -1.0);
  nh.param("sdf_map/map_origin_z", z_origin, -1.0);

  nh.param("sdf_map/inflation_xy", mp_->inflation_xy_, -1.0);
  nh.param("sdf_map/inflation_z", mp_->inflation_z_, -1.0);

  nh.param("sdf_map/inflate_local", mp_->inflate_local_, false);
  ...
  mp_->map_origin_ = Eigen::Vector3d(x_origin, y_origin, z_origin);
  ...
  nh.param("sdf_map/p_pred_occ", mp_->p_pred_occ_, 0.80);
  ...
  mp_->pred_occupancy_log_ = logit(mp_->p_pred_occ_);
  mp_->unknown_flag_ = 0.01;
  cout << "hit: " << mp_->prob_hit_log_ << ", miss: " << mp_->prob_miss_log_
       << ", min: " << mp_->clamp_min_log_ << ", max: " << mp_->clamp_max_log_
       << ", thresh: " << mp_->min_occupancy_log_ << ", pred thresh: " << mp_->pred_occupancy_log_ << endl;

  // Initialize data buffer of map
  int buffer_size = mp_->map_voxel_num_(0) * mp_->map_voxel_num_(1) * mp_->map_voxel_num_(2);
  md_->occupancy_buffer_ = vector<double>(buffer_size, mp_->clamp_min_log_ - mp_->unknown_flag_);
  md_->occupancy_buffer_inflate_global_ = vector<double>(buffer_size, mp_->clamp_min_log_ - mp_->unknown_flag_);
...
void SDFMap::getPointCloud(pcl::PointCloud<pcl::PointXYZ> &cloud){
...
void SDFMap::inputPointCloud(
  ...
  for (int i = 0; i < point_num; ++i) {
    ...
    while (caster_->nextId(idx)) {
      ...
      // setCacheOccupancy(idx, toAddress(idx), 1);
    }
  }
  ...
  while (!md_->cache_voxel_.empty()) {
    ...
    // Update global inflate map
    Eigen::Vector3i tmp_idx = md_->cache_voxel_idx_.front();
    md_->cache_voxel_idx_.pop();

    // 1. Do the same to voxels in the inflation region
    // If unknown, make it known
    if (md_->occupancy_buffer_inflate_global_[adr] < mp_->clamp_min_log_ - 1e-3)
      md_->occupancy_buffer_inflate_global_[adr] = mp_->clamp_min_log_;
    // Bound between min and max
    md_->occupancy_buffer_inflate_global_[adr] = std::min(
        std::max(md_->occupancy_buffer_inflate_global_[adr] + log_odds_update, mp_->clamp_min_log_),
        mp_->clamp_max_log_);
    ...
    if (getOccupancy(tmp_idx) == OCCUPIED) {
      for (const auto& it_n: local_infla_array_) {
        ...
        if (isInMap(neighbor_idx)) {
          ...
          md_->occupancy_buffer_inflate_global_[neighbor_adr] = getOriOccupancy(tmp_idx);
          // if (md_->occupancy_buffer_inflate_global_[neighbor_adr] < mp_->clamp_min_log_ - 1e-3)
          //   md_->occupancy_buffer_inflate_global_[neighbor_adr] = mp_->clamp_min_log_;
          // // Bound between min and max
          // md_->occupancy_buffer_inflate_global_[neighbor_adr] = std::min(
          //     std::max(md_->occupancy_buffer_inflate_global_[neighbor_adr] + log_odds_update, mp_->clamp_min_log_),
          //     mp_->clamp_max_log_);
        }
      }
    }
...
double SDFMap::getKnownVolume() {
  return mr_->known_volume;
}
