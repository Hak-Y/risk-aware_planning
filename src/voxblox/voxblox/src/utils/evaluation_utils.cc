...
template <>
bool isObservedVoxel(const UncertaintyVoxel& voxel) {
  return voxel.feature==0.0;
}
...
template <>
FloatingPoint getVoxelSdf(const UncertaintyVoxel& voxel) {
  return voxel.distance;
}
...
template <>
void setVoxelSdf(const FloatingPoint sdf, UncertaintyVoxel* voxel) {
  CHECK_NOTNULL(voxel);
  voxel->distance = sdf;
}
...
template <>
void setVoxelWeight(const FloatingPoint weight, UncertaintyVoxel* voxel) {
  CHECK_NOTNULL(voxel);
  voxel->observed = weight > 0.;
}
