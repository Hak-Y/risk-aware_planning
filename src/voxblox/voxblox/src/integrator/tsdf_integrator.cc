...
void MergedTsdfIntegrator::bundleRays(
  ...
  while (index_getter->getNextIndex(&point_idx)) {
    ...
    if (is_clearing) {
      (*clear_map)[voxel_index].push_back(point_idx); // SH: voxel_map is hash map. each voxel in voxel_map contain point_idx of pointcloud
    } else {
...
void MergedTsdfIntegrator::integrateVoxel(
  ...
  for (const size_t pt_idx : kv.second) { // pt_index  is idx for a point in pointcloud
