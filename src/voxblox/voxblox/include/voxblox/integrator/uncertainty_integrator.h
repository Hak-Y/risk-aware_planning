#ifndef VOXBLOX_INTEGRATOR_UNCERTAINTY_INTEGRATOR_H_
#define VOXBLOX_INTEGRATOR_UNCERTAINTY_INTEGRATOR_H_
...
/**
 * Builds an UNCERTAINTY layer out of a given TSDF layer. For a description of this
...
class UncertaintyIntegrator {
 public:
  ...
  UncertaintyIntegrator(const Config& config, Layer<TsdfVoxel>* tsdf_layer,
  ...
  /**
   *Update from a TSDF layer in batch, clearing the current UNCERTAINTY layer in the
   * process.
   */
  ...
  /**
   * The core of UNCERTAINTY updates: processes a queue of voxels to get the minimum
  ...
  float getUncertaintyMaxDistance() const { return config_.max_distance_m; }
 ...
 protected:
  ...
  Layer<UncertaintyVoxel>* uncertainty_layer_;

  /**
   * Open Queue for incremental updates. Contains global voxel indices
   * for the UNCERTAINTY layer.
   */
...
#endif  // VOXBLOX_INTEGRATOR_UNCERTAINTY_INTEGRATOR_H_
