...
class Layer {
 public:
  ...
  inline typename BlockType::Ptr getBlockPtrByIndex(const BlockIndex& index) {
    ...
    typename BlockHashMap::iterator it = block_map_.find(index);
 // Print the size of block_map_
  if (block_map_.size()==0)
    // std::cout << "Size of block_map_: " << block_map_.size() << std::endl;

    // std::cout << "my it: " << it << std::endl;
    if (it != block_map_.end()) {
      // std::cout << "my it2: " << it->second << std::endl;
      return it->second;
    } else {
      // std::cout << "my it3: " << typename BlockType::Ptr() << std::endl;
  ...
  inline BlockIndex computeBlockIndexFromCoordinates(
      const Point& coords) const {
        // std::cout << "my coords: " << coords << std::endl;
  ...
  void getAllUpdatedBlocks(Update::Status bit, BlockIndexList* blocks) const {
    ...
    for (const std::pair<const BlockIndex, typename BlockType::Ptr>& kv :
         block_map_) {
          ...
          // std::cout << "kv.second->updated(): " << kv.second->updated() << std::endl;
          // std::cout << "kv.second->updated()[bit]: " << kv.second->updated()[bit] << std::endl;
