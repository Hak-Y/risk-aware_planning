...
inline bool visualizeDistanceIntensityUncertaintyVoxels(const UncertaintyVoxel& voxel,
                                                 const Point& /*coord*/,
                                                 double* intensity) {
  CHECK_NOTNULL(intensity);
  if (voxel.observed) {
...
inline bool visualizeDistanceIntensityUncertaintyVoxelsSlice(
    const UncertaintyVoxel& voxel, const Point& coord, unsigned int free_plane_index,
    FloatingPoint free_plane_val, FloatingPoint voxel_size, double* intensity) {
  CHECK_NOTNULL(intensity);
...
inline bool visualizeFreeUncertaintyVoxels(const UncertaintyVoxel& voxel,
                                    const Point& /*coord*/, float min_distance,
                                    double* intensity) {
  if (voxel.observed && voxel.distance >= min_distance) {
    *intensity = voxel.distance;
...
inline void createDistancePointcloudFromUncertaintyLayer(
    const Layer<UncertaintyVoxel>& layer,
    pcl::PointCloud<pcl::PointXYZI>* pointcloud) {
  CHECK_NOTNULL(pointcloud);
  createColorPointcloudFromLayer<UncertaintyVoxel>(
      layer, &visualizeDistanceIntensityUncertaintyVoxels, pointcloud);
}
...
inline void createFreePointcloudFromUncertaintyLayer(
    const Layer<UncertaintyVoxel>& layer, float min_distance,
    pcl::PointCloud<pcl::PointXYZI>* pointcloud) {
  CHECK_NOTNULL(pointcloud);
  createColorPointcloudFromLayer<UncertaintyVoxel>(
      layer,
      std::bind(&visualizeFreeUncertaintyVoxels, ph::_1, ph::_2, min_distance, ph::_3),
      pointcloud);
}
...
inline void createDistancePointcloudFromUncertaintyLayerSlice(
    const Layer<UncertaintyVoxel>& layer, unsigned int free_plane_index,
    FloatingPoint free_plane_val, pcl::PointCloud<pcl::PointXYZI>* pointcloud) {
  CHECK_NOTNULL(pointcloud);
  // Make sure that the free_plane_val doesn't fall evenly between 2 slices.
  // Prefer to push it up.
  ...
  createColorPointcloudFromLayer<UncertaintyVoxel>(
      layer,
      std::bind(&visualizeDistanceIntensityUncertaintyVoxelsSlice, ph::_1, ph::_2,
                free_plane_index, free_plane_val, layer.voxel_size(), ph::_3),
      pointcloud);
}
