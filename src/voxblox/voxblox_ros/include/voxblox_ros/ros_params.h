...
inline UncertaintyMap::Config getUncertaintyMapConfigFromRosParam(
    const ros::NodeHandle& nh_private) {
  UncertaintyMap::Config uncertainty_config;

  const TsdfMap::Config tsdf_config = getTsdfMapConfigFromRosParam(nh_private);
  uncertainty_config.uncertainty_voxel_size = tsdf_config.tsdf_voxel_size;
  uncertainty_config.uncertainty_voxels_per_side = tsdf_config.tsdf_voxels_per_side;
...
inline UncertaintyIntegrator::Config getUncertaintyIntegratorConfigFromRosParam(
    const ros::NodeHandle& nh_private) {
  UncertaintyIntegrator::Config uncertainty_integrator_config;

  TsdfIntegratorBase::Config tsdf_integrator_config =
      getTsdfIntegratorConfigFromRosParam(nh_private);

  uncertainty_integrator_config.min_distance_m =
      tsdf_integrator_config.default_truncation_distance / 2.0;

  nh_private.param("uncertainty_euclidean_distance",
                   uncertainty_integrator_config.full_euclidean_distance,
                   uncertainty_integrator_config.full_euclidean_distance);
  nh_private.param("uncertainty_max_distance_m", uncertainty_integrator_config.max_distance_m,
                   uncertainty_integrator_config.max_distance_m);
  nh_private.param("uncertainty_min_distance_m", uncertainty_integrator_config.min_distance_m,
                   uncertainty_integrator_config.min_distance_m);
  nh_private.param("uncertainty_default_distance_m",
                   uncertainty_integrator_config.default_distance_m,
                   uncertainty_integrator_config.default_distance_m);
  nh_private.param("uncertainty_min_diff_m", uncertainty_integrator_config.min_diff_m,
                   uncertainty_integrator_config.min_diff_m);
  nh_private.param("clear_sphere_radius",
                   uncertainty_integrator_config.clear_sphere_radius,
                   uncertainty_integrator_config.clear_sphere_radius);
  nh_private.param("occupied_sphere_radius",
                   uncertainty_integrator_config.occupied_sphere_radius,
                   uncertainty_integrator_config.occupied_sphere_radius);
  nh_private.param("uncertainty_add_occupied_crust",
                   uncertainty_integrator_config.add_occupied_crust,
                   uncertainty_integrator_config.add_occupied_crust);
  if (uncertainty_integrator_config.default_distance_m <
      uncertainty_integrator_config.max_distance_m) {
    uncertainty_integrator_config.default_distance_m =
        uncertainty_integrator_config.max_distance_m;
  }

  return uncertainty_integrator_config;
}
