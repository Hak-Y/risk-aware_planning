...
class EvalData(object):
    def __init__(self):
        ...
        # Parse parameters
        self.ns_planner = rospy.get_param('~ns_planner', "/planner/planner_node")
        ...
        if self.evaluate:
            ...
            self.ns_voxblox = rospy.get_param('~ns_voxblox', "/planner/voxblox_node")
            ...
            self.eval_data_file = open(os.path.join(self.eval_directory, "voxblox_data.csv"), 'w')
            self.eval_writer = csv.writer(self.eval_data_file, delimiter=',', quotechar='|', quoting=csv.QUOTE_MINIMAL, lineterminator='\n')
            ...
            self.ue_out_sub = rospy.Subscriber("/voxel_grid/output", PointCloud2, self.ue_out_callback, queue_size=10)
    ...
    def launch_simulation(self):
        ...
        if self.startup_timeout > 0.0:
            try:
                rospy.loginfo("Waiting for 'simulation_manager/simulation_ready' message...")
                rospy.wait_for_message("simulation_manager/simulation_ready", String, self.startup_timeout)
                rospy.loginfo("'simulation_manager/simulation_ready' message received.")
            except rospy.ROSException as e:
                self.stop_experiment("Simulation startup failed (timeout after " + str(self.startup_timeout) + "s). Error: " + str(e))
                return
        else:
            rospy.wait_for_message("simulation_manager/simulation_ready", String)
            rospy.loginfo("'simulation_manager/simulation_ready' message received.")
        ...
        # Evaluation init
        if self.evaluate:
            self.writelog("Successfully started the simulation.")
            ...
            bag_expr = re.compile(r'tmp_bag_\d{4}-\d{2}-\d{2}-\d{2}-\d{2}-\d{2}\.bag.')  # Default names
            bags = [b for b in os.listdir(os.path.join(os.path.dirname(self.eval_directory), "tmp_bags")) if bag_expr.match(b)]
        ...
        # Finish
        rospy.loginfo("\n" + "*" * 39 + "\n* Successfully started the simulation! *\n" + "*" * 39)
