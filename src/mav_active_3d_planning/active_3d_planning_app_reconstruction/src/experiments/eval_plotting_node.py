...
class EvalPlotting(object):
    ...
    have the format hardcoded in the eval_data_node.
    """
    def __init__(self):
        # Parse parameters
    ...
    def run_single_evaluation(self, target_dir):
        ...
        if self.create_plots:
            ...
            if os.path.isfile(os.path.join(target_dir, "voxblox_data.csv")):
                ...
                if len(data_voxblox.get('RosTime', [])) > 1:
                    # 'MeanError'가 존재하지 않을 경우에도 최소한의 평가를 수행
                    if 'MeanError' in data_voxblox:
                        self.plot_sim_overview(data_voxblox, target_dir)
                    else:
                        rospy.loginfo(
                            "'MeanError' not found in 'voxblox_data.csv', performing minimal evaluation.")
                        # 최소한의 평가 수행 (예: CPUTime, NPointclouds 등 다른 메트릭 사용)
                        if 'CPUTime' in data_voxblox and 'NPointclouds' in data_voxblox:
                            self.plot_minimal_evaluation(data_voxblox, target_dir)
                else:
                    rospy.loginfo(
    ...
    def plot_minimal_evaluation(self, data, target_dir):
        rospy.loginfo("Creating Graphs: MinimalEvaluation")
        ...
        cpu_use = np.zeros(np.shape(cpu_time))
        for i in range(len(cpu_time) - 1):
            cpu_use[i] = (cpu_time[i + 1]) / (x[i + 1] - x[i])
        cpu_use[-1] = cpu_use[-2]
        cpu_use = np.repeat(cpu_use, 2)

        fig, axes = plt.subplots(2, 1)
        axes[0].plot(x, np.cumsum(np.array(data['NPointclouds'], dtype=float)), 'k-')
        axes[0].set_ylabel('Processed Pointclouds [-]')
        axes[0].set_xlim(left=0, right=x[-1])
        axes[0].set_ylim(bottom=0)
        
        x = np.repeat(x, 2)
        x = np.concatenate((np.array([0]), x[:-1]))
        axes[1].plot(x, cpu_use, 'k-')
        axes[1].set_ylabel('Simulated CPU usage [cores]')
        axes[1].set_xlabel("Simulated Time [%s]" % unit)
        axes[1].set_ylim(bottom=0)
        axes[1].set_xlim(left=0, right=x[-1])
        
        plt.suptitle("Minimal Evaluation Overview")
        fig.set_size_inches(10, 10, forward=True)

        save_name = os.path.join(target_dir, "graphs", "MinimalEvaluation.png")
        plt.savefig(save_name, dpi=300, format='png', bbox_inches='tight')
        self.writelog("Created graph 'MinimalEvaluation'.")
