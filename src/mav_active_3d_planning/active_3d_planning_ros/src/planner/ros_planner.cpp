...
void RosPlanner::planningLoop() {
  ...
  return_srv_ = nh_private_.advertiseService("toggle_returnning",
                                          &RosPlanner::returnSrvCallback, this);
  running_ = true;
  std::clock_t timer;
  while (::ros::ok()) {
    if (return_planning_)
    {
      Eigen::Vector3d cur_pose = getCurrentPosition();
      Eigen::Vector3d base = Eigen::Vector3d(0,0,1.5);
      double xy_norm = (base.head<2>() - cur_pose.head<2>()).norm();
      if (xy_norm < 1.0){
        printInfo("R2B finish!");
        break;
      }
    }
    if (planning_) {
  ...
  if (markers.getMarkers()[0].action == VisualizationMarker::OVERWRITE) {
    ...
    if (count > markers.getMarkers().size()) {
      ...
      for (int i = markers.getMarkers().size(); i < count; ++i) {
        ...
        empty_marker.header.frame_id = "odom";
...
bool RosPlanner::returnSrvCallback(std_srvs::SetBool::Request& req,
                                std_srvs::SetBool::Response& res) {
  res.success = true;
  if (req.data) {
    initializePlanning();
    return_planning_ = true;
    ROS_INFO("Started return_planning.");
  } else {
    return_planning_ = false;
    ROS_INFO("Stopped return_planning.");
  }
  return true;
}
