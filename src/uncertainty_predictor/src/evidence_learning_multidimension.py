...
def main():
    x_dim=3
    y_dim=2
    x_train, y_train = my_data(-4,0, 1000,x_dim,y_dim)
    train_data = TensorDataset(x_train, y_train)
    train_loader = DataLoader(train_data, batch_size=100, shuffle=True, num_workers=0)
    train_iter = iter(train_loader)
    x_test, y_test = my_data(-7, 7, 1000, x_dim,y_dim,train=False)
...
def my_data(x_min, x_max, n, x_dim, y_dim,train=True):
    x = np.linspace(x_min, x_max, n)
    x = np.expand_dims(x, -1).astype(np.float32)
    x = np.tile(x, 3)
    sigma = 3 * np.ones_like(x) if train else np.zeros_like(x)
    y1 = (x ** 3 + np.random.normal(0, sigma).astype(np.float32)).sum(1)
    y2 = (x + np.random.normal(0, sigma).astype(np.float32)).sum(1)
...
def plot_predictions(x_train, y_train, x_test, y_test, y_pred, n_stds=4, kk=0):
    ...
    fig, axs = plt.subplots(x_dim, y_dim, figsize=(15, 10))
    ...
    for i in range(x_dim):
        for j in range(y_dim):
            ...
            c_x_train = x_train[:, i]
            c_mu = mu[:, j]
            c_var = np.minimum(var, 1e3)[:, j]
            c_y_test = y_test[:, j]
            c_y_train = y_train[:, j]
            
            # Plot on the corresponding subplot
            axs[i, j].scatter(c_x_train, c_y_train, s=1., c='#463c3c', zorder=0, label="Train")
            axs[i, j].plot(c_x_test, c_y_test, 'r--', zorder=2, label="True")
            axs[i, j].plot(c_x_test, c_mu, color='#007cab', zorder=3, label="Pred")
            axs[i, j].plot([-4, -4], [-150, 150], 'k--', alpha=0.4, zorder=0)
            axs[i, j].plot([+4, +4], [-150, 150], 'k--', alpha=0.4, zorder=0)
            
            for k in np.linspace(0, n_stds, 4):
                axs[i, j].fill_between(
                    c_x_test, (c_mu - k * c_var), (c_mu + k * c_var),
                    alpha=0.3,
                    edgecolor=None,
                    facecolor='#00aeef',
                    linewidth=0,
